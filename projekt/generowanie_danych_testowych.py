...
def insert_pracownicy(n=10):
    for _ in range(n):
        query = """
            ...
            VALUES 
                (%(imie)s, %(nazwisko)s, %(email)s, %(telefon)s, %(data_zatrudnienia)s,
                %(stanowisko)s, %(poziom_certyfikacji)s, %(kontakt_awaryjny_imie)s,
                %(kontakt_awaryjny_telefon)s)
        """
        params = {
            'imie': fake.first_name(),
            'nazwisko': fake.last_name(),
            'email': fake.email(),
            'telefon': fake.phone_number().replace(" ", ""),
            ...
            'kontakt_awaryjny_telefon': fake.phone_number().replace(" ", "")
        }
...
rodzaje_wydarzen_data = [
    ('Survival amatorski', 131, 'Weekendowy kurs przetrwania', 800, 16, 15, 5, 'Góry niskie'),
    ('Survival zaawansowany', 131, 'Tygodniowy kurs przetrwania', 2500, 168, 10, 3, 'Góry wysokie'),
    ('Inscenizowane porwanie', 135, 'Symulowane porwanie na wieczór kawalerski', 1500, 6, 1, 1, 'Góry niskie'),
    ('Zwiedzanie TPN', 134, 'Wycieczka po Tatrzańskim Parku Narodowym', 100, 3, 30, 5, 'Góry'),
    ('Ekspedycja górska', 135, 'Wyprawa w wysokie góry', 5000, 72, 8, 4, 'Góry wysokie'),
    ('Wspinaczka wielkościanowa', 135, 'Całodniowa wspinaczka na długich trasach', 800, 10, 6, 2, 'Góry wysokie'),
    ('Team survival', 132, 'Survival dla firm z elementami team buildingu', 400, 8, 40, 10, 'Góry'),
    ('Zimowa integracja', 132, 'Całodniowa zimowa aktywność w górach', 450, 8, 35, 10, 'Góry niskie')
]
...
def insert_rodzaje_wydarzen():
    query = """
        ...
        VALUES 
            (%(nazwa)s, %(kategoria_id)s, %(opis)s, %(cena)s,
            %(czas)s, %(max_ucz)s, %(min_ucz)s, %(lokalizacja)s)
    """
...
def insert_klienci(n=50):
    for _ in range(n):
        query = """
            ...
            VALUES 
                (%(imie)s, %(nazwisko)s, %(email)s, %(telefon)s, %(adres)s,
                %(data_urodzenia)s, %(stan_zdrowia)s, %(kontakt_imie)s,
                %(kontakt_tel)s, %(kontakt_rel)s)
        """
        params = {
            'imie': fake.first_name(),
            'nazwisko': fake.last_name(),
            ...
            'kontakt_tel': fake.phone_number().replace(" ", ""),
...
def insert_rezerwacje(n=100):
    ...
    for _ in range(n):
        ...
        cena = float(cena_podstawowa) * uczestnicy * random.uniform(0.9, 1.2)
        cena = round(cena, 2)
        ...
        query = """
            ...
            VALUES 
                (%(klient_id)s, %(rodzaj_id)s, %(data_rez)s,
                %(data_wyd)s, %(status)s, %(liczba_ucz)s,
                %(cena)s, %(zgoda)s)
        """
        params = {
...
def insert_personel_wydarzenia(n=150):
    ...
    for _ in range(n):
        query = """
            ...
            VALUES 
                (%(rezerwacja_id)s, %(pracownik_id)s, %(rola)s, %(uwagi)s)
        """
        params = {
...
def insert_koszty_wydarzenia(n=200):
    ...
    for _ in range(n):
        ...
        query = """
            ...
            VALUES 
                (%(rezerwacja_id)s, %(typ)s, %(kwota)s, %(opis)s, %(data)s)
        """
...
def insert_transakcje_finansowe(n=300):
    ...
    for rezerwacja in rezerwacje:
        ...
        query = """
            ...
            VALUES 
                (%(rezerwacja_id)s, %(data)s, %(kwota)s, %(typ)s,
                %(metoda)s, %(status)s, %(uwagi)s)
        """
...
def insert_opinie(n=80):
    ...
    for rezerwacja in random.sample(rezerwacje, min(n, len(rezerwacje))):
        ...
        query = """
            ...
            VALUES 
                (%(rezerwacja_id)s, %(ocena)s, %(komentarz)s, %(data)s)
        """
    ...
    for nazwa, opis in sprzet_lista:
        for _ in range(random.randint(1, 5)):  # Multiple items of the same type
            query = """
                ...
                VALUES 
                    (%(nazwa)s, %(opis)s, %(ilosc)s, %(stan)s,
                    %(ostatni_przeglad)s, %(nastepny_przeglad)s)
            """
...
def insert_sprzet_wydarzenia(n=200):
    ...
    for _ in range(n):
        query = """
            ...
            VALUES 
                (%(rezerwacja_id)s, %(sprzet_id)s, %(ilosc)s, %(uwagi)s)
        """
